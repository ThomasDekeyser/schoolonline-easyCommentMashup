...
// @include		https://bsvisitatie.schoolonline.be/PuntenboekLeerkracht/PuntenboekLeerkrachtEdit.php?*
// @version     1.0.0
// ==/UserScript==
...
function initPopupFields() {
	...
	$('input[id^="EvaluatieMomentLijnCommentaar"]').each(function() {
...
function addStyle() {
	...
	GM_addStyle(".popBox-container { display:none; background-color: #fff; border:4px solid #000; padding:10px; text-align:left; z-index: 9999; -webkit-border-radius: .5em;  -moz-border-radius: .5em; border-radius: .5em; -webkit-box-shadow: 0 0px 25px rgba(0,0,0,.9); -moz-box-shadow: 0 0px 25px rgba(0,0,0,.9); box-shadow: 0 1px 2px rgba(0,0,0,.2); border: 5px solid rgb(000, 0, 0); border: 5px solid rgba(000, 0, 0, .9); position: fixed;}");
            ...
            obj.focus(function () {
                ...
                $.each(arrayFixedList, function(index,value) {
					if (value.trim().length != 0) {
						var checkString = '';
						if(variableString.indexOf(value) >= 0) {
								checkString='checked';
						}
						popBoxContainer.children(".popBox-container-checkboxes").append('<input type="checkbox" class="popBox-checkbox-'+index+'" value="'+index+'" '+checkString+'> '+value+'</input><br/>');
					}
				});
                popBoxContainer.css({ position: 'fixed', margin: 0, top: (top > 0 ? top : 0) + 'px', left: (left > 0 ? left : 0) + 'px' });
                popBoxContainer.children(".popBox-container-checkboxes").children('.popBox-checkbox-0').focus();
                ...
                popBoxContainer.children('.done-button').children('#'+doneButtonName).click(function () {
                    if (change) {
                        ...
                        var newString = '';
                        $(this).parent().parent().children('.popBox-container-checkboxes').children(':checked').each(function(index) {
							if (newString.length > 0 ) {
								newString = newString + '\n' + arrayFixedList[$(this).val()];
							}else {
								newString = arrayFixedList[$(this).val()];
							}
                        });                                              
